...
Status update (2026-07-04): the sibling is deployed in production on a ~35B local MoE, including the judgment-imitation layer — variance probes replacing self-reported escalation, a competence map fed by nightly frontier grading, and the framing library's distillation loop. The loop is live: within a day, the majority of new framings were distilled by the frontier model from answered escalation-queue items rather than hand-seeded. Several of those (sanitized) now ship as this repo's `templates/framings.template.md` — the distillation pipeline exporting judgment back into the zero-dependency version.
